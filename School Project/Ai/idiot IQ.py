...
genai.configure(api_key="AIzaSyD9NH6Znlu-fOJkmEShXkdtm4fXj-2_gC4")
...
# Variables
chat_lines = [("Hi, I'm your personal AI assistant, how can I help you?", False)]
user_input = ""
scroll_offset = 0
SPACING = 10
...
def calculate_chat_height(chat_lines):
    """Calculates the total height of the chat content."""
    height = 0
    for line, _ in chat_lines:
        wrapped_lines = wrap_text(line, font, scrollable_area.width)
        height += len(wrapped_lines) * font.get_height() + SPACING
    return height
...
def render_chat(surface, chat_lines, scroll_offset):
    ...
    for line, is_user in chat_lines:
        ...
        for wrapped_line in wrap_text(line, font, scrollable_area.width):
            if scrollable_area.top <= y <= scrollable_area.bottom - font.get_height():
        ...
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_RETURN:
                if user_input.strip():
                    ...
                    user_input = ""
                    # Automatically snap to the bottom when a new message is added
                    chat_height = calculate_chat_height(chat_lines)
                    if chat_height > scrollable_area.height:
                        scroll_offset = max(0, chat_height - scrollable_area.height)
                ...
                chat_height = calculate_chat_height(chat_lines)
                if chat_height > scrollable_area.height:
                    scroll_offset = min(scroll_offset + SCROLL_SPEED, chat_height - scrollable_area.height)
            else:
